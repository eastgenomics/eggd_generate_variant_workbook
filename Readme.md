...
`--exclude_columns` (string): Columns of VCF to exclude from output xlsx

`--include_columns` (string): Columns of VCF to only include in output xlsx
...
`--add_samplename` (bool): Determines if to add samplename as first column in each sheet (default: False)

`--sheets_names` (list): Names to use for workbook sheets, these MUST be the same number as the number of vcfs passed and in the same order. If not given, and if there is 1 vcf passed the sheet will be named `variants`, else if multiple vcfs are passed the name prefix of the vcf will be usedNames to use for sheets where multiple VCFs are passe
...
`--merge`_vcfs (bool): Determines if to merge multiple VCFs to one sheet (default: False)
